...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function open(){ results.classList.add("show"); input.setAttribute("aria-expanded","true"); }
  function close(){ results.classList.remove("show"); input.setAttribute("aria-expanded","false"); activeIndex=-1; }
  ...
  async function fetchAll(signal){
    ...
    console.log("[search] counts:",
      { products: p.data?.length||0, users: u.data?.length||0, sales: s.data?.length||0 });
  ...
  function render(data, q){
    ...
    const products = (data.products||[]).filter(p => (p.name||"").toLowerCase().includes(qlc)).slice(0,8);
    const users    = (data.users||[]).filter(u =>
                      (u.username||"").toLowerCase().includes(qlc) ||
                      (u.email||"").toLowerCase().includes(qlc)).slice(0,8);
  ...
  input.addEventListener("keydown", (e)=>{
    if(e.key==="ArrowDown"){ e.preventDefault(); move(1); }
      ...
      if(results.classList.contains("show") && activeIndex>=0 && flatItems[activeIndex]){
        e.preventDefault(); flatItems[activeIndex].click();
      }
